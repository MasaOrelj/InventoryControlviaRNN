"""Backfill reps 10-39 for the spikes-off arms (d=10 and d=25, K-1=30), to
bring them to the same 40-rep depth as the spikes-on arms -- see
conversation: the four-way curvature comparison needs matched precision, not
two arms at n=10 and two at n=40.

Run: python -m scripts.RNN_K30_NoSpikes_Backfill <d>
"""
import functools
import os
import sys
...
CSV_PATH = os.path.join(os.path.dirname(__file__), "..", "results", f"rnn_k30_d{N_DIMS}_nospikes_experiment.csv")
...
if __name__ == "__main__":
    print(f"Spikes-off backfill: d={N_DIMS}, reps {min(NEW_REPS)}-{max(NEW_REPS)}")
    base_row = _base_row()
    ...
    for rep in NEW_REPS:
        ...
        print(f"  d={N_DIMS} rep {rep} done ({time.time() - t0:.1f}s elapsed)")
