"""One-off: Laguerre (degree 2), d=25, L=10 (default contract), lambda=0.5,
10 reps -- fills the gap between the existing 0.1 and 1.0 grid points, same
motivation as RNN's lambda=0.3/0.7 spot checks (see conversation).

Run: python -m scripts.Laguerre_Ridge_Lambda_Experiment_LSweep
"""
import functools
import os
import time
...
CSV_PATH = os.path.join(os.path.dirname(__file__), "..", "results", "laguerre_d25_lambda05_experiment.csv")
...
if __name__ == "__main__":
    print(f"Laguerre d=25, lambda={LAMBDA}, {N_REPS} reps")
    base_row = _base_row()
    ...
    for rep in range(N_REPS):
        ...
        print(f"  rep {rep + 1}/{N_REPS} done ({time.time() - t0:.1f}s elapsed, this rep {duration_sec:.1f}s)")
