"""Difference-in-differences arm for test (5) (see conversation): the d=25
spikes-off result alone shows spikes matter AT d=25, but the original finding
was a d=10-vs-d=25 ASYMMETRY. Attributing that asymmetry to spike sparsity
needs the d=10 arm too: if d=10 was "less spike-dominated to begin with", its
price-optimal df* should shift much LESS when spikes are killed than d=25's
did (+13.4, from ~4.5 to ~17.9). A comparably large shift at d=10 would mean
spikes matter everywhere, not specifically explaining why d=25 differs from
d=10.

Same lambda grid, seeds, K-1=30 as the d=25 spikes-off run.

Run: python -m scripts.RNN_K30_D10_NoSpikes_Experiment
"""
import functools
import os
import time
...
CSV_PATH = os.path.join(os.path.dirname(__file__), "..", "results", "rnn_k30_d10_nospikes_experiment.csv")
