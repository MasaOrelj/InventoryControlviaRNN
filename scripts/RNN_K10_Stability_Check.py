"""Backfill reps 10-39 for K-1=10 at the four (d, lambda) cells discussed in
the df(lambda) in [3.5,10] comparison (see conversation): d=1 (lambda in
{0.001, 0.01}), d=10 (lambda=0.1), d=25 (lambda=0.1). Brings K-1=10 up to the
same 40-rep depth already available for K-1=30, for a fair, well-powered
comparison against K-1=30's own best price/SD cells per dimension.

Run: python -m scripts.RNN_K10_Stability_Check
"""
import functools
import os
import time
...
CSV_PATH = os.path.join(os.path.dirname(__file__), "..", "results", "rnn_k10_stability_check_experiment.csv")
...
N_HIDDEN = 10
CELLS = [(1, 0.001), (1, 0.01), (10, 0.1), (25, 0.1)]
...
if __name__ == "__main__":
    print(f"K-1=10 backfill: cells={CELLS}, reps {min(NEW_REPS)}-{max(NEW_REPS)}")
    print(f"logging to {os.path.abspath(CSV_PATH)}\n")

    for n_dims, lam in CELLS:
        ...
        for rep in NEW_REPS:
            ...
            print(f"  d={n_dims:2d} lambda={lam} rep {rep} done ({time.time() - t0:.1f}s elapsed)")
        print()
