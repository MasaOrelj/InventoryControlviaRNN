"""Same as E1_Inventory_Mode_Experiment_RNN.py (fixed vs. variable inventory
formulation, all 3 dims, M_t sweep, both inventory modes, 10 reps), but
parameterized by K-1 on the command line, writing to its own CSV -- lets
several K-1 values run as separate OS processes in parallel. Mirrors
RNN_Ridge_Lambda_Experiment_OneL.py's pattern.

Run: python -m scripts.E1_Inventory_Mode_Experiment_RNN_OneK <K-1>
"""
import os
import sys
import time
...
if __name__ == "__main__":
    print(f"E1 (RNN, K-1={N_HIDDEN}): M_t in {M_T_VALUES}, M_e={M_E}, {N_REPS} reps, dims={DIMS}, "
