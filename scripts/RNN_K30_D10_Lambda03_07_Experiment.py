"""One-off: RNN, K-1=30, d=10, lambda in {0.3, 0.7}, 40 reps (0-39, same seed
formula as everywhere else in this project), to compare against the already-
existing 40-rep lambda=0.1 data (df~10.2) -- target df(lambda) in [4.5, 6.5],
which lambda=0.3 (df~6.50) and lambda=0.7 (df~4.28) roughly bracket.

Run: python -m scripts.RNN_K30_D10_Lambda03_07_Experiment
"""
import functools
import os
import time
...
CSV_PATH = os.path.join(os.path.dirname(__file__), "..", "results", "rnn_k30_d10_lambda03_07_experiment.csv")
...
if __name__ == "__main__":
    print(f"K-1=30, d=10, lambda in {LAMBDA_VALUES}, reps 0-39")
    base_row = _base_row()
