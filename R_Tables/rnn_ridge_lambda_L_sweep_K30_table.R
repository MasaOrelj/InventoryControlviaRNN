...
load_prices <- function(L) {
  if (L == 1) {
    ...
    df <- read_csv("results/rnn_ridge_lambda_experiment.csv", show_col_types = FALSE)
  } else {
    df <- read_csv("results/rnn_ridge_lambda_L_sweep_experiment.csv", show_col_types = FALSE) %>%
      filter(L == !!L)
  }
  df %>% filter(basis_n_hidden == 30) %>% mutate(L = !!L)
}

raw <- bind_rows(lapply(L_VALUES, load_prices)) %>%
  filter(is.na(ridge_lambda) | !(ridge_lambda %in% c(10, 15, 20)))

sig <- read_csv("results/rnn_ridge_lambda_L_sweep_K30_significance.csv", show_col_types = FALSE) %>%
...
# rows -- mirrors rnn_ridge_lambda_table.R's K-1-group layout, with L in
# place of K-1. Bold means that cell's price AND sd are both statistically
# tied with that (n_dims, L) block's own best price / best sd (see
# Compute_RNN_L_Sweep_K30_Significance.py) -- reference block is per (d, L),
# not pooled across L.
...
make_dim_block <- function(d) {
  caption <- paste0(
    "RNN basis, $K-1=30$, $d=", d, "$: plain least squares vs.\\ a ridge $\\lambda$ grid, ",
    ...
    "(exact sign-flip test, $\\alpha=0.05$) from that $(d,L)$ block's own best price and best SD."
  )
  paste0(
    "\\begin{table}[htbp]\n",
    "\\caption{\\label{tab:rnn_ridge_lambda_L_sweep_K30_d", d, "}", caption, "}\n",
    "\\scriptsize\n",
...
writeLines(combined, "R_Tables/rnn_ridge_lambda_L_sweep_K30_table.tex")
